...
FEATURES = [
    'debit_eau_m3h', 'puissance_totale_kw',
...
TARGETS = ['debit_eau_m3h', 'puissance_totale_kw', 'cout_total_fcfa']
...
def predict_simulation(padded: List[HoraireInput]) -> np.ndarray:
    moy_debit = float(np.mean([h.debit_eau_m3h       for h in padded[-12:]]))
...
def predict_simple(
    ...
    for i in range(WINDOW_IN):
        ...
        fake.append(HoraireInput(
            ...
            debit_eau_m3h         = float(np.clip(130000*profil + np.random.normal(0,3000), 65000, 240000)),
...
from orchestrateur import Orchestrateur, EtatStation

orche = Orchestrateur(api_url="http://localhost:8000")

@app.post("/decide", tags=["Orchestrateur"])
def decide(etat: dict):
    """
    Cycle décisionnel complet — coordonne LSTM + DQN + Autoencoder.
    Retourne la décision optimale pour la station.
    """
    return orche.cycle_dict(etat)


@app.post("/decide/simple", tags=["Orchestrateur"])
def decide_simple(
    niveau_chateau : float = 65.0,
    niveau_bache   : float = 72.0,
    heure          : int   = 14,
    ...
    """Test rapide de l'Orchestrateur sans body JSON."""
    etat = EtatStation(
        niveau_chateau_pct   = niveau_chateau,
        niveau_bache_pct     = niveau_bache,
        heure                = heure,
        temperature_C        = temperature,
        coupure_sonabel      = coupure,
        puissance_solaire_kw = solaire,
        tarif_fcfa_kwh       = 84.0 if heure < 17 else 165.0,
        plage_tarifaire      = 'HC' if heure < 17 else 'HP',
    )
    return orche.cycle(etat).__dict__
